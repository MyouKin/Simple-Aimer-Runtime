...
void Detector::save(const Armor & armor) const
{
  ...
  auto img_path = fmt::format("{}/{}_{}.jpg", save_path_, ARMOR_NAMES[static_cast<size_t>(armor.name)], file_name);
...
cv::Mat draw_detection(const cv::Mat & bgr_img, const std::list<Armor> & armors)
{
  auto detection = bgr_img.clone();
  ...
  for (const auto & armor : armors) {
    ...
    if (armor.points.size() >= 4) {
      ...
      cv::line(detection, armor.points[0], armor.points[1], {0, 255, 0}, 2);
      cv::line(detection, armor.points[1], armor.points[2], {0, 255, 0}, 2);
      cv::line(detection, armor.points[2], armor.points[3], {0, 255, 0}, 2);
      cv::line(detection, armor.points[3], armor.points[0], {0, 255, 0}, 2);
    }

    // 绘制兵种标签
    auto & name_str = ARMOR_NAMES[static_cast<int>(armor.name)];
    auto & type_str = ARMOR_TYPES[static_cast<int>(armor.type)];
    auto info = fmt::format("{:.2f} {} {}", armor.confidence, name_str, type_str);

    auto text_pos = armor.points.size() >= 4
    ...
    cv::putText(detection, info, text_pos, cv::FONT_HERSHEY_SIMPLEX,
                0.5, {0, 255, 255}, 1);
  }

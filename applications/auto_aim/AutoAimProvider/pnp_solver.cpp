/// @file pnp_solver.cpp
/// @brief PnP 3D 姿态解算器实现 — 移植自 spr_vision_try/tasks/auto_aim/solver.cpp
  ...
  : R_gimbal2world_(Eigen::Matrix3d::Identity())
{
  ...
  tools::logger()->info("[PnPSolver] Calibration loaded.");
}

void PnPSolver::setRgimbal2world(const Eigen::Quaterniond & q)
{
  Eigen::Matrix3d R_imubody2imuabs = q.toRotationMatrix();
  R_gimbal2world_ = R_gimbal2imubody_.transpose() * R_imubody2imuabs * R_gimbal2imubody_;
}

void PnPSolver::solve(Armor & armor) const
{
  if (armor.points.size() < 4) {
    tools::logger()->warn("[PnPSolver] Armor has < 4 points, skipping PnP.");
    return;
  }

  // 1. 选择 3D 模型点（大/小装甲板）
  const auto & object_points = (armor.type == ArmorType::big)
    ? BIG_ARMOR_POINTS : SMALL_ARMOR_POINTS;

  // 2. solvePnP: 2D 角点 → camera 系位姿
  cv::Vec3d rvec, tvec;
  cv::solvePnP(
    object_points, armor.points, camera_matrix_, distort_coeffs_,
    rvec, tvec, false, cv::SOLVEPNP_IPPE);

  // 3. 坐标变换: camera → gimbal
  Eigen::Vector3d xyz_in_camera;
  cv::cv2eigen(tvec, xyz_in_camera);
  armor.xyz_in_gimbal = R_camera2gimbal_ * xyz_in_camera + t_camera2gimbal_;
  armor.xyz_in_world  = R_gimbal2world_ * armor.xyz_in_gimbal;

  // 4. 姿态变换: camera → gimbal → world
  cv::Mat rmat;
  ...
  // 6. 平衡不做 yaw 优化（pitch 假设不成立）
  auto is_balance = (armor.type == ArmorType::big) &&
                    (armor.name == ArmorName::three || armor.name == ArmorName::four ||
                     armor.name == ArmorName::five);
  if (is_balance) return;

  // 7. Yaw 角度优化: ±70° 搜索最低重投影误差
  optimize_yaw(armor);
}
...
double PnPSolver::armor_reprojection_error(
